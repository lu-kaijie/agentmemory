...
本变更先建立必需的 OpenAI-compatible LLM/Embedding provider 和配置/健康检查能力。系统没有可用 AI provider 时不能进入可用运行状态，以保证产品从第一版开始就是 AI native。
...
- LLM 与 embedding provider 均为必需配置，缺少 base URL、API key 或 model 时服务启动失败。
...
- `llm-provider`: 必需 LLM 与 embedding provider 的配置、接口、OpenAI-compatible 实现和可用性检查。
