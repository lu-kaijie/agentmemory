...
AgentMemory 已经具备 CLI、REST、LLM processing 和搜索能力，但 agent 还没有统一的使用协议。下一步需要提供一个项目 Skill，让不同 coding agent 能稳定知道何时查询、何时保存、如何避免高频调用拖慢当前任务。
...
- `agent-skill`: AgentMemory Skill 的安装位置、使用规则、CLI/REST 调用策略、低频触发策略和安全写入约束。
