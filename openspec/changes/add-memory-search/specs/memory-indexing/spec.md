...
#### Scenario: Vector index is updated on write

- **WHEN** searchable document 写入索引
- **THEN** 系统调用 embedding provider 并把向量写入 LanceDB
...
Rebuild MUST 清空并重建 FTS5 与 LanceDB 索引。Repair MUST 尝试补建缺失或 failed 的 index jobs。
...
#### Scenario: Repair failed index jobs

- **WHEN** 用户触发 index repair
- **THEN** 系统重试 failed 或缺失的 index jobs
