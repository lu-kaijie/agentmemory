...
CLI MUST 提供 `agentmemory context "<query>"` 和 `agentmemory context "<query>" --json`。REST MUST 提供 `POST /agentmemory/context`。两个接口 MUST 支持 `query`、`tokenBudget`、`limit`、`project`、`language` 和 `sourceTypes`。
