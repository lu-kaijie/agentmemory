...
- CLI：`agentmemory remember`、`agentmemory recall`、`agentmemory observe`、`agentmemory context`、`agentmemory wiki`、`agentmemory serve`。
- REST API：`/agentmemory/observe`、`/agentmemory/remember`、`/agentmemory/search`、`/agentmemory/smart-search`、`/agentmemory/export`、`/agentmemory/health`。
...
- LLM provider 不可用时，原始 observation 仍能保存，摘要任务状态可追踪。
...
- `src/index.ts`：服务装配入口，加载配置、初始化 provider、状态层、REST、Viewer 和后台任务。
- `src/config.ts`：环境变量、默认值和配置校验。
- `src/providers/`：OpenAI-compatible LLM provider、embedding provider、fallback 和测试 fake provider。
...
- `src/cli.ts`：统一 CLI 入口，供 Skill 和 shell-capable agent 调用。
...
选择原因：Typer 基于类型标注生成 CLI，和 FastAPI/Pydantic 的模型风格一致，适合第一版快速实现 `serve`、`observe`、`remember`、`recall`、`context`、`wiki`、`export`、`doctor` 等命令。
...
- 第一版图谱定位：轻量知识图谱 Viewer。
- 图谱生成：LLM 从 memory、summary、Wiki 页面和重要 observation 中抽取节点和边。
- 图谱存储：`KV.graphNodes`、`KV.graphEdges`。
- 图谱 API：`/agentmemory/graph/query`、`/agentmemory/graph/stats`。
...
- 核心函数单元测试。
- LLM/embedding fake provider 测试。
- REST API 集成测试。
- CLI JSON 输出测试。
...
- `KV.relations`、`KV.graphNodes`、`KV.graphEdges`：关系图数据，用于 Viewer 展示和后续图谱检索。
...
- 什么时候调用 `agentmemory recall` 搜索历史经验。
...
```bash
agentmemory recall "<task or question>" --json
agentmemory context --project "<project-path>" --json
```
...
```bash
agentmemory wiki show --topic "<topic>" --json
agentmemory wiki search "<question>" --json
```
...
`recall` 和 `context` 返回结构化 JSON。agent 应遵守：
...
如果配置了 `AGENTMEMORY_SECRET`，请求必须带 bearer token。

### REST

REST 路径统一以 `/agentmemory/*` 开头。非公开端点在 `AGENTMEMORY_SECRET` 存在时必须校验 bearer token。
...
首版 provider 使用 OpenAI-compatible API 形态，默认支持 OpenAI，也允许接入兼容 OpenAI 协议的模型服务。高级配置可以后续增加向量维度、rerank model 和图谱抽取策略。
