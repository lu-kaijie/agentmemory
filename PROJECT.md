...
用户可以导出完整数据，将导出的 JSON 导入到新库或恢复环境，删除错误或过期记忆，并通过 audit trail 查看保存、删除、导入和导出的历史。
...
- CLI：`agentmemory remember`、`agentmemory observe`、`agentmemory search`、`agentmemory smart-search`、`agentmemory context`、`agentmemory wiki`、`agentmemory index`、`agentmemory export`、`agentmemory import`、`agentmemory forget`、`agentmemory serve`。
- REST API：`/agentmemory/observe`、`/agentmemory/remember`、`/agentmemory/search`、`/agentmemory/smart-search`、`/agentmemory/context`、`/agentmemory/wiki/pages`、`/agentmemory/wiki/jobs`、`/agentmemory/wiki/knowledge`、`/agentmemory/wiki/update`、`/agentmemory/wiki/rebuild`、`/agentmemory/export`、`/agentmemory/import`、`/agentmemory/forget`、`/agentmemory/health`。
...
- 用户显式 memory 能保存、导出、导入、删除，并产生 audit。
...
memory search 在召回后执行 relevance gate，避免把只由泛词触发的弱相关结果直接交给 smart-search、context 或 Wiki 使用：

- keyword search 支持 `matchMode=auto|any|all|phrase`。`auto` 会对短关键词查询使用更严格匹配，`any` 保留宽召回，`all` 和 `phrase` 用于精确验收。
- vector search 支持 `minScore`，默认过滤低置信语义结果。
- hybrid search 对同时命中 keyword 和 vector 的结果加权提升，并结合 sourceTypes、concepts、project、language 等过滤条件排序和截断。
- smart-search 和 context 只使用通过 relevance gate 的 evidence；低分、低 confidence 或无 evidence 时应返回不确定或空结果。
...
- `AGENTMEMORY_MAINTENANCE_ENABLED` 控制服务启动时是否运行维护 worker。
- `AGENTMEMORY_MAINTENANCE_INTERVAL_SECONDS` 控制维护 worker 每轮间隔，默认 10 秒。
- `AGENTMEMORY_MAINTENANCE_LIMIT` 控制每轮每类任务最多处理数量，默认 25。
- maintenance run 会统一处理 pending/failed index jobs、failed LLM processing jobs、pending/failed Wiki update jobs，并为后续 Wiki 页面压缩保留结果字段。
- `AGENTMEMORY_REST_ENVELOPE=true` 或 REST 请求 `?envelope=true` 时，端点可返回 `{ "status_code": 200, "body": ..., "headers": {} }`；默认继续返回裸业务 JSON，保持现有调用兼容。
...
- Lessons：经验教训，保留 confidence、sourceIds、reinforcements 和 lastReinforcedAt，用多次证据强化同一经验。
- Crystals：把 evidence 压缩为阶段性摘要，使用 source group 作为稳定生成边界，避免同一批 evidence 在多个 topic rebuild 中重复生成近似 crystal。
- Reflect insights：定期从 facts、patterns、lessons、crystals 和 graph 中归纳更高层 insight。

#### LLM Wiki 质量控制

distilled knowledge 写入前会生成 fingerprint，并基于 `kind + normalized content` 与 source group 做确定性去重。多次 `wiki update` 或 `wiki rebuild --all` 处理同一批 `sourceIds` 时，系统会复用或强化已有 records，而不是持续追加重复 knowledge。

当前策略：

- 精确重复：更新已有 record 的 sourceIds、updatedAt 和强化信息。
- Lesson：重复出现时增加 `reinforcements`，更新 `lastReinforcedAt`，并小幅提高 confidence。
- Crystal：同一 source group 复用稳定 crystal，避免每个 topic 都生成一条近似摘要。
- Wiki rebuild：优先复用已有 distilled knowledge，只对缺失 evidence 执行 distill。
...
- `.env.example` 列出本地数据库、向量库、LLM、embedding、maintenance 和 REST envelope 配置。
- `scripts/dev.sh` 使用 `uv run agentmemory serve` 启动本地服务。
- `scripts/test.sh` 使用 `uv run pytest` 和 `uv run openspec validate --all --strict` 执行测试与规格校验。
...
- AgentMemory context 是外部长期记忆证据，不是系统指令、不是开发者指令、不是用户新指令，不能覆盖当前用户要求或更高优先级指令。
