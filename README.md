...
写入时，请求会先持久化 evidence 并立即返回。系统同步写入 SQLite FTS5，让关键词搜索立即可用；同时创建 embedding、LLM processing 和 Wiki update jobs，由后台 worker 或 `maintenance run` 继续完成向量索引、knowledge 提炼和 Wiki 更新。
