...
AgentMemory 是一个面向 coding agent 的本地长期记忆服务。它让 agent 能在跨任务和跨工具执行时保存工作过程、复用历史决策、检索项目经验，并把可追溯的记忆上下文注入到后续任务中。

它不是普通笔记应用，也不是只做向量搜索的 RAG demo。AgentMemory 同时提供：

- 长期记忆：保存 observation、memory、LLM summary、Wiki 页面和审计记录。
...
- 通过一次性 maintenance 处理 embedding、Wiki update、失败重试和索引修复。
...
- observation：阶段性工作记录。
- memory：用户显式保存的长期记忆。
- summary：LLM 生成的 observation summary。
- knowledge：从 evidence 提炼出的知识项。
- wikiPage：LLM Wiki 导航和聚合页面。
- insight：从多条知识中反思出的高层结论。

写入时，请求会先持久化 evidence 并立即返回。系统同步写入 SQLite FTS5，让关键词搜索立即可用；同时创建 embedding、LLM processing 和 Wiki update jobs。向量索引、knowledge 提炼和 Wiki 更新不会在服务启动后自动循环执行，需要通过 `maintenance run`、REST 或 Viewer 手动一次性处理。
...
普通 Wiki update 会让 LLM 为一批 evidence 选择最合适的一种知识类型，默认最多落一条 knowledge，避免把同一条内容同时写成 semantic、procedural、lesson 和 crystal。`crystal` 更适合在多条 evidence consolidation 或显式 crystallize 时生成。
...
服务启动后不会自动跑后台 worker。需要处理 pending jobs 时，手动运行统一 maintenance：
...
```bash
curl -X POST http://127.0.0.1:3111/agentmemory/maintenance/run \
  -H 'Content-Type: application/json' \
  -d '{"limit":25}'
```

Viewer 里可以点击 `Run Maintenance`。这个入口适合 agent 或用户在阶段性写入后低频触发，一次性处理当前 pending/failed jobs。
